...
現在の collector 登録 provider は以下です。

- OpenAI
- Anthropic
- Gemini

Gemini は Groq の source review 結果を踏まえ、Terms / source suitability を先に確認したうえで追加しました。
Primary source は HTML ではなく、公式 docs の Markdown/text source `https://ai.google.dev/gemini-api/docs/pricing.md.txt` です。
対象は Paid Tier / Standard / text input-output pricing に限定し、Preview / tiered pricing / media / Batch / Flex / Priority などは除外します。
...
- parser 検証
  - provider ごとに最小 synthetic fixture と unit test を持ち、source 構造変更で静かに壊れないようにする
  - fixture は実ページ全文コピーではなく、parser に必要な最小構造だけを残す
...
候補 provider の source review は `docs/provider-source-policy.md` に記録します。
